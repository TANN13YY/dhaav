...
export const onUserCreated = functions.auth.user().onCreate(async (user) => {
  ...
  const dhaavId = await db.runTransaction(async (transaction) => {
    ...
    let nextId = 1;
    if (configDoc.exists) {
      nextId = (configDoc.data()?.userCount || 0) + 1;
    }
...
export const onUserDeleted = functions.auth.user().onDelete(async (user) => {
  ...
  const deleteDocs: admin.firestore.DocumentSnapshot[] = [userDoc];
  
  // Also delete AuthMap entry
  const authMapDoc = await db.collection('AuthMap').doc(uid).get();
  if (authMapDoc.exists) deleteDocs.push(authMapDoc);
...
export const claimWelcomeBonus = functions.https.onCall(async (data, context) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (userData?.welcomeRPClaimed === true) {
...
export const adminClearMockData = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('permission-denied', 'Must be authenticated');
  }
...
export const adminSimulateTerritoryLoss = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('permission-denied', 'Must be authenticated');
  }
  ...
  await db.runTransaction(async (transaction) => {
    ...
    transaction.set(battleRef, {
      ...
      defenderName: userDoc.data()?.username || 'You',
      rpStolen: rpToLose,
...
export const submitRun = functions.https.onCall(async (data, context) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const parsedCoordinates = (pathCoordinates || []).map((c: any) => {
...
export const onPendingTerritoryClaim = functions.firestore.document('PendingTerritoryClaims/{docId}').onCreate(async (snap, context) => {
  ...
  if (!Array.isArray(enemyDeductions) || typeof selfOverlapRP !== 'number') return;
  ...
  if (totalStolen > 5000 || selfOverlapRP > 5000) return; // Hack attempt
...
export const onRunDeleted = functions.firestore.document('RunHistory/{runId}').onDelete(async (snap, context) => {
  const data = snap.data();
  const uid = data.owner_id;
  const totalRP = data.totalRP || 0;
  
  if (!uid || totalRP <= 0) return;
